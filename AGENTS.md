...
- **Nunca corras `php artisan test` con la configuración cacheada.** Si existe `bootstrap/cache/config.php`, Laravel ignora el `.env` y las variables de `phpunit.xml`, y la suite —que borra tablas con `Schema::dropIfExists`— se ejecuta contra PostgreSQL y destruye la base real. Antes de probar, siempre `php artisan config:clear`. `tests/TestCase.php` aborta la suite si la conexión no es SQLite en memoria, pero la costumbre es la primera defensa.
- Después de cada `git pull` en la VM corre `php artisan optimize:clear`, y sólo entonces vuelve a cachear si hace falta (`config:cache`, `route:cache`, `view:cache`, en ese orden). Las vistas se recompilan solas al cambiar de fecha, pero el caché de rutas no: si el despliegue trae rutas nuevas, `bootstrap/cache/routes-*.php` se queda con las viejas y la aplicación corre a medias —`route()` revienta con «Route [nombre] not defined» y los middleware nuevos ni siquiera se aplican, así que una zona que debía quedar cerrada sigue abierta—.
