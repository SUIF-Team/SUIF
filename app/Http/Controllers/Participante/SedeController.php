...
class SedeController extends Controller
{
    ...
    {
        $estado = (array) $request->session()->get('suif.participante.estado', []);

        if (!empty($estado['sede_seleccionada'])) {
            return view('participante.sede', [
                'confirmada' => true,
                'sede' => (array) $request->session()->get('suif.participante.sede', []),
            ]);
        }
    ...
    public function seleccionar(Request $request)
    {
        ...
        if (!$sede) {
            return redirect()->route('participante.sede.index')
                ->withErrors(['sede' => 'Selecciona una sede válida.']);
        }

        $request->session()->put('suif.participante.sede', $sede);
        $request->session()->put('suif.participante.estado.sede_seleccionada', true);
    ...
    public function reiniciar(Request $request)
    {
        ...
        $request->session()->forget('suif.participante.sede');
        $request->session()->put('suif.participante.estado.sede_seleccionada', false);
