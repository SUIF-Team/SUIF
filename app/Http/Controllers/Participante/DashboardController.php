...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        /* Sustituye este estado de sesión por consultas reales cuando existan modelos. */
        $estado = array_merge([
            'preregistro_completo' => false,
            'referencia_generada' => false,
            'pago_estado' => 'sin_cargar', // sin_cargar, revision, rechazado, validado
    ...
    private function construirPasos(array $estado)
    {
        ...
        $pasos[] = $this->paso(1, 'Pre-registro',
            ...
            $pre ? 'Revisar' : 'Continuar', 'participante.preregistro', true);
        ...
        $pasos[] = $this->paso(2, 'Obtener referencia',
            ...
            $ref ? 'Consultar' : 'Generar', 'participante.referencia', $pre);
        ...
        $pasos[] = $this->paso(4, 'Selección de sede y horario',
            ...
            $sede ? 'Consultar' : 'Seleccionar', 'participante.sede', $pagoValidado);
        ...
        $pasos[] = $this->paso(5, 'Resultados',
            ...
            'Consultar', 'participante.resultados', $resultado);
    ...
    private function pasoPago($referenciaGenerada, $pago)
    {
        if (!$referenciaGenerada) {
            return $this->paso(3, 'Pago', 'Disponible después de generar tu referencia.', 'locked', 'Pendiente', 'Continuar', 'participante.pago', false);
        }
        if ($pago === 'validado') {
            return $this->paso(3, 'Pago', 'Tu comprobante fue validado por el administrador.', 'completed', 'Validado', 'Consultar', 'participante.pago', true);
        }
        if ($pago === 'revision') {
            return $this->paso(3, 'Pago', 'Tu comprobante está siendo revisado.', 'review', 'En revisión', 'Consultar', 'participante.pago', true);
        }
        if ($pago === 'rechazado') {
            return $this->paso(3, 'Pago', 'El comprobante tiene observaciones. Corrígelo y vuelve a cargarlo.', 'warning', 'Requiere atención', 'Corregir', 'participante.pago', true);
        }
        return $this->paso(3, 'Pago', 'Carga tu comprobante para que el administrador valide el pago.', 'active', 'Disponible', 'Continuar', 'participante.pago', true);
    }
