...
- Toda la zona administrativa exige sesión iniciada y un privilegio del catálogo. Hay tres tipos de administrador: **Admin UIF** (pre-registro y documentación), **Admin DEC** (pagos y referencias bancarias) y **Superusuario** (todos los módulos). Cada quien aterriza en su bandeja al iniciar sesión y el tablero sólo le pinta lo que puede abrir. Las altas se hacen desde `/admin/administradores`; `database/scripts/suif_roles_administrativos.sql` es requisito para que los privilegios existan.
...
| Administración | `/admin/dashboard`, personas, pagos, referencias, documentos, sedes, administradores y resultados | Todo el grupo exige `auth` y un privilegio; cada módulo pide el suyo. Dashboard, bandejas y el módulo de administradores conectados a la base; certificados y resultados continúan pendientes. |
